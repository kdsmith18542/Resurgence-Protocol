...
**Resurgence Protocol** is a Proof-of-Dormancy staking system that allows holders of abandoned ("dead") ERC-20 tokens to stake them and earn RESURGE, a governance and rewards token.
...
| 3 | `claimAndRestakeTo` in `DeadCoinStakingPool` not extensively tested | Low | Acknowledged | Function exists for composability but has minimal integration test coverage. |

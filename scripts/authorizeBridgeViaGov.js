/**
 * authorizeBridgeViaGov.js
 *
 * Governance proposal to call authorizeBridge() on RewardDistributor
 * for the NEW CrossChainReceiver (which has the supportsInterface fix).
 *
 * Usage:
 *   npx hardhat run scripts/authorizeBridgeViaGov.js --network arbitrumSepolia
 */
...
async function main() {
  ...
  const alreadyAuthorized = await distributor.authorizedBridges(NEW_RECEIVER);
  if (alreadyAuthorized) {
    log("✅ New CrossChainReceiver already authorized as bridge — nothing to do.");
    return;
  }

  const calldata = distributor.interface.encodeFunctionData("authorizeBridge", [NEW_RECEIVER]);

  const targets     = [DISTRIBUTOR_ADDRESS];
  const values      = [0n];
  const calldatas   = [calldata];
  const description = [
    "# Authorize new CrossChainReceiver as bridge on RewardDistributor",
    "",
    "Calls authorizeBridge() on RewardDistributor to authorize the fixed CrossChainReceiver",
    "(v2, with supportsInterface fix so CCIP Router actually invokes ccipReceive).",
    "",
    `RewardDistributor: ${DISTRIBUTOR_ADDRESS}`,
    `New CrossChainReceiver: ${NEW_RECEIVER}`,
  ].join("\n");
  ...
  if (state === S.Active) {
    ...
    if (!alreadyVoted) {
      log("Voting For...");
      const voteTx = await governance.castVoteWithReason(proposalId, 1, "Authorize new CrossChainReceiver with supportsInterface fix");
      await voteTx.wait();
  ...
  const authorized = await distributor.authorizedBridges(NEW_RECEIVER);
  if (authorized) {
    log("✅ New CrossChainReceiver authorized as bridge successfully.");
  } else {
    log("⚠️  Bridge not authorized — verify manually.");
  }
}

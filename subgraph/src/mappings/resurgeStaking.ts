...
} from '../../generated/ResurgeStakingPool/ResurgeStakingPool'
import {
  User,
  StakingPool,
  StakingPosition,
...
export function handleResurgeStaked(event: StakedEvent): void {
  ...
  if (pool == null) {
    // If pool is not found, it's the native pool which might not be in the registry
    // We should create it if it doesn't exist
    pool = new StakingPool(Bytes.fromHexString(poolAddress.toHexString()))
    pool.poolAddress = poolAddress
    pool.deadCoinToken = Address.fromString("0x0000000000000000000000000000000000000000") // Native RESURGE
    pool.rewardRatePerSecond = ZERO_BI // Initial, will be updated by events
...
export function handleResurgeUnstaked(event: UnstakedEvent): void {
  let userAddress = event.params.user
  let amount = event.params.amount
  let poolAddress = event.address
...
export function handleResurgeRewardsClaimed(event: RewardsClaimedEvent): void {
  let userAddress = event.params.user
  let amount = event.params.amount
  let poolAddress = event.address

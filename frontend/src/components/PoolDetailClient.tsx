...
import { useErc20Balance, useErc20Symbol } from '@/hooks/useTokenData';
...
export default function PoolDetailClient() {
  ...
  const deadCoinAddress = poolData?.deadCoinToken as `0x${string}` | undefined;
  const { data: symbol } = useErc20Symbol(deadCoinAddress);
  const { data: userBalance } = useErc20Balance(deadCoinAddress, userAddress as `0x${string}`);
  ...
  const displaySymbol = (symbol as string) || '???';
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 font-mono text-sm">{displaySymbol} &middot; {poolAddress?.slice(0, 10)}...</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard label="Total Staked" value={formatTokenAmount(totalStaked)} sub={displaySymbol} />
        <StatCard label="Your Stake" value={formatTokenAmount(userStaked)} sub={displaySymbol} />
      ...
      <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
        ...
        <div className="space-y-2 text-sm text-gray-400">
          ...
          <div className="flex justify-between"><span>Total Staked</span><span className="text-white">{formatTokenAmount(totalStaked)} {displaySymbol}</span></div>
        </div>
      </div>

      <StakeModal isOpen={showStake} onClose={() => setShowStake(false)} poolAddress={poolAddress as `0x${string}`} deadCoinAddress={deadCoinAddress as `0x${string}`} userBalance={(userBalance as bigint) || 0n} userStaked={userStaked} tokenSymbol={displaySymbol} />
      <UnstakeModal isOpen={showUnstake} onClose={() => setShowUnstake(false)} poolAddress={poolAddress as `0x${string}`} stakedAmount={userStaked} tokenSymbol={displaySymbol} />
